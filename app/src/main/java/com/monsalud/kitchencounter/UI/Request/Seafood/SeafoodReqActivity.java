...
public class SeafoodReqActivity extends AppCompatActivity
        implements SeafoodRecyclerAdapter.SeafoodRecyclerListener, SeafoodReqDetailFragment.OnAddToRequestsListener {

    SeafoodRecyclerAdapter mSeafoodRecyclerAdapter;
    KCRepository mRespository;

    ConstraintLayout seafoodreq;
    int mId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_seafood_req);

        mRespository = new KCRepository(getApplication());
        mSeafoods = mRespository.getAllSeafoods();
        seafoodreq = findViewById(R.id.seafoodreq);

        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.detailLayout_reqseafood, new SeafoodReqDetailFragment());
        ft.replace(R.id.listLayout_reqseafood, new SeafoodReqListFragment());
        ft.commit();

        mTvRequestReview = findViewById(R.id.tvReviewRequests_reqseafood);
        mTvRequestReview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(SeafoodReqActivity.this, RequestReviewActivity.class);
    ...
    public void onSeafoodRecyclerSent(CharSequence seafoodName, CharSequence seafoodType, int productID, InventoryItem.InventoryUnit unit) {
        SeafoodReqDetailFragment.updateFragment(seafoodName, seafoodType, productID, unit);
    }
    ...
    public void onAddToRequests() {
        ...
        mUnit = SeafoodReqDetailFragment.mUnit;
//        int inventoryUnitPosition = SeafoodReqDetailFragment.mSpinUnit.getSelectedItemPosition();
...
//        LocalDate nowLD = Converters.fromEpochMilliToLocalDate(nowMillis);
        ...
        try {
            mProdID = Integer.parseInt(String.valueOf(SeafoodReqDetailFragment.mTvProductID.getText()));

            mAmount = Double.parseDouble(String.valueOf(SeafoodReqDetailFragment.mEtAmount.getText()));
        } catch(Exception e) {
            showErrorMsg();
        }
    ...
    private void showErrorMsg() {
        Snackbar errorInsertInventory = Snackbar.make(seafoodreq, "There was an error submitting this item", Snackbar.LENGTH_LONG);
    ...
    private void confirmInventory() {
        Snackbar confirmationInventory = Snackbar.make(seafoodreq, "This item has been submitted", Snackbar.LENGTH_LONG);
