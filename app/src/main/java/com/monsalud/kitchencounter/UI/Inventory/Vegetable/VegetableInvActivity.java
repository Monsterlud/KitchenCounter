...
public class VegetableInvActivity extends AppCompatActivity
    implements VegetableRecyclerAdapter.VegetableRecyclerListener, VegetableInvDetailFragment.OnAddToInventoryListener {

    TextView mTvInventoryReview;
    VegetableRecyclerAdapter mVegetableRecyclerAdapter;
    KCRepository mRespository;
    ConstraintLayout vegetableinv;
    int mId;

    private InventoryItem.InventoryUnit mUnit;
    private InventoryItem.InventoryLocation mLocation;

    public static List<Vegetable> mVegetables;
    private int mProdID;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_vegetable_inv);

        mTvInventoryReview = findViewById(R.id.tvReviewInv_produce);
        mTvInventoryReview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(VegetableInvActivity.this, InventoryReviewActivity.class);
                startActivity(intent);
            }
        });
        vegetableinv = findViewById(R.id.vegetableinv);

        mRespository = new KCRepository(getApplication());
        mVegetables = mRespository.getAllVegetables();

        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.detailLayout_produce, new VegetableInvDetailFragment());
        ft.replace(R.id.listLayout_produce, new VegetableInvListFragment());
        ft.commit();
    }

    @Override
    public void onVegRecyclerSent(CharSequence vegName, CharSequence vegVarietal, int productID, InventoryItem.InventoryUnit unit) {
        VegetableInvDetailFragment.updateFragment(vegName, vegVarietal, productID, unit);
    }
    ...
    @Override
     public void onAddToInventory() {
        ...
        mUnit = VegetableInvDetailFragment.mUnit;
//        int inventoryUnitPosition = VegetableInvDetailFragment.mSpinUnit.getSelectedItemPosition();
...
//        else mUnit = null;
        ...
        int inventoryLocationPosition = VegetableInvDetailFragment.mSpinLocation.getFirstVisiblePosition();
        ...
        //Get a LocalDate for a datestamp
        ...
        try {
            mProdID = Integer.parseInt(String.valueOf(VegetableInvDetailFragment.mTvProductID.getText()));
        } catch(Exception e) {
            showErrorMsg();
        }
       ...
       try {
           InventoryItem item = new InventoryItem(
                   ...
                   nowLD,
                   Double.parseDouble(String.valueOf(VegetableInvDetailFragment.mEtAmount.getText())),
                   mLocation,
                   false, true);
    ...
    private void showErrorMsg() {
        Snackbar errorInsertInventory = Snackbar.make(vegetableinv, "There was an error submitting this item", Snackbar.LENGTH_LONG);
    ...
    private void confirmInventory() {
        Snackbar confirmationInventory = Snackbar.make(vegetableinv, "This item has been submitted", Snackbar.LENGTH_LONG);
