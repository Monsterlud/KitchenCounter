...
public class FruitInvActivity extends AppCompatActivity
    implements FruitRecyclerAdapter.FruitRecyclerListener, FruitInvDetailFragment.OnAddToInventoryListener {

    TextView mTvInventoryReview;
    FruitRecyclerAdapter mFruitRecyclerAdapter;
    KCRepository mRespository;
    ConstraintLayout fruitinv;
    int mId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fruit_inv);

        mTvInventoryReview = findViewById(R.id.tvReviewInv_fruit);
        mTvInventoryReview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(FruitInvActivity.this, InventoryReviewActivity.class);
                startActivity(intent);
            }
        });
        fruitinv = findViewById(R.id.fruitinv);

        mRespository = new KCRepository(getApplication());
        mFruits = mRespository.getAllFruits();

        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.detailLayout_fruit, new FruitInvDetailFragment());
        ft.replace(R.id.listLayout_fruit, new FruitInvListFragment());
        ft.commit();
    }
    ...
    public void onFruitRecyclerSent(CharSequence fruitName, CharSequence fruitVarietal, int productID, InventoryItem.InventoryUnit unit) {
        FruitInvDetailFragment.updateFragment(fruitName, fruitVarietal, productID, unit);
    }
    ...
    public void onAddToInventory() {
        ...
        mUnit = FruitInvDetailFragment.mUnit;
//        int inventoryUnitPosition = FruitInvDetailFragment.mSpinUnit.getSelectedItemPosition();
...
//        else mUnit = null;
        ...
        int inventoryLocationPosition = FruitInvDetailFragment.mSpinLocation.getFirstVisiblePosition();
        ...
        try {
            mProdID = Integer.parseInt(String.valueOf(FruitInvDetailFragment.mTvProductID.getText()));
        } catch(Exception e) {
            showErrorMsg();
        }
        ...
        try {
            InventoryItem item = new InventoryItem(
                    ...
                    nowLD,
                    Double.parseDouble(String.valueOf(FruitInvDetailFragment.mEtAmount.getText())),
                    mLocation,
                    false, true);
    ...
    private void showErrorMsg() {
        Snackbar errorInsertInventory = Snackbar.make(fruitinv, "There was an error submitting this item", Snackbar.LENGTH_LONG);
    ...
    private void confirmInventory() {
        Snackbar confirmationInventory = Snackbar.make(fruitinv, "This item has been submitted", Snackbar.LENGTH_LONG);
