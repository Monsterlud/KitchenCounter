...
public class DairyReqActivity extends AppCompatActivity
        implements DairyRecyclerAdapter.DairyRecyclerListener, DairyReqDetailFragment.OnAddToRequestsListener {

    DairyRecyclerAdapter mDairyRecyclerAdapter;
    KCRepository mRespository;

    ConstraintLayout dairyreq;
    int mId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dairy_req);

        mRespository = new KCRepository(getApplication());
        mDairies = mRespository.getAllDairy();
        dairyreq = findViewById(R.id.dairyreq);

        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.detailLayout_reqdairy, new DairyReqDetailFragment());
        ft.replace(R.id.listLayout_reqdairy, new DairyReqListFragment());
        ft.commit();

        mTvRequestReview = findViewById(R.id.tvReviewRequests_reqdairy);
        mTvRequestReview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(DairyReqActivity.this, RequestReviewActivity.class);
    ...
    public void onDairyRecyclerSent(CharSequence dairyName, CharSequence dairyType, int productID, InventoryItem.InventoryUnit unit) {
        DairyReqDetailFragment.updateFragment(dairyName, dairyType, productID, unit);
    }
    ...
    public void onAddToRequests() {
        ...
        mUnit = DairyReqDetailFragment.mUnit;
//        int inventoryUnitPosition = DairyReqDetailFragment.mSpinUnit.getSelectedItemPosition();
...
//        LocalDate nowLD = Converters.fromEpochMilliToLocalDate(nowMillis);
        ...
        try {
            mProdID = Integer.parseInt(String.valueOf(DairyReqDetailFragment.mTvProductID.getText()));

            mAmount = Double.parseDouble(String.valueOf(DairyReqDetailFragment.mEtAmount.getText()));
        } catch(Exception e) {
            showErrorMsg();
        }
    ...
    private void showErrorMsg() {
        Snackbar errorInsertInventory = Snackbar.make(dairyreq, "There was an error submitting this item", Snackbar.LENGTH_LONG);
    ...
    private void confirmInventory() {
        Snackbar confirmationInventory = Snackbar.make(dairyreq, "This item has been submitted", Snackbar.LENGTH_LONG);
