...
public class VegetableReqActivity extends AppCompatActivity
        implements VegetableRecyclerAdapter.VegetableRecyclerListener, VegetableReqDetailFragment.OnAddToRequestsListener {


    VegetableRecyclerAdapter mVegetableRecyclerAdapter;
    KCRepository mRespository;

    ConstraintLayout producereq;
    int mId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_vegetable_req);

        mRespository = new KCRepository(getApplication());
        mVegetables = mRespository.getAllVegetables();
        producereq = findViewById(R.id.producereq);

        FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
        ft.replace(R.id.detailLayout_reqproduce, new VegetableReqDetailFragment());
        ft.replace(R.id.listLayout_reqproduce, new VegetableReqListFragment());
        ft.commit();

        mTvRequestReview = findViewById(R.id.tvReviewRequests_reqproduce);
        mTvRequestReview.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(VegetableReqActivity.this, RequestReviewActivity.class);
    ...
    public void onVegRecyclerSent(CharSequence vegName, CharSequence vegVarietal, int productID, InventoryItem.InventoryUnit unit) {
        VegetableReqDetailFragment.updateFragment(vegName, vegVarietal, productID, unit);
    }
    ...
    public void onAddToRequests() {
        ...
        mUnit = VegetableReqDetailFragment.mUnit;
//        int inventoryUnitPosition = VegetableReqDetailFragment.mSpinUnit.getSelectedItemPosition();
...
//        LocalDate nowLD = Converters.fromEpochMilliToLocalDate(nowMillis);
        ...
        try {
            mProdID = Integer.parseInt(String.valueOf(VegetableReqDetailFragment.mTvProductID.getText()));

            mAmount = Double.parseDouble(String.valueOf(VegetableReqDetailFragment.mEtAmount.getText()));
        } catch(Exception e) {
            showErrorMsg();
        }
    ...
    private void showErrorMsg() {
        Snackbar errorInsertInventory = Snackbar.make(producereq, "There was an error submitting this item", Snackbar.LENGTH_LONG);
    ...
    private void confirmInventory() {
        Snackbar confirmationInventory = Snackbar.make(producereq, "This item has been submitted", Snackbar.LENGTH_LONG);
